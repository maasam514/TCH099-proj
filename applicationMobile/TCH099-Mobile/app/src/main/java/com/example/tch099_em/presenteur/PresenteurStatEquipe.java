...
public class PresenteurStatEquipe {
    ...
    public void obtenirNomEquipe(int idEquipe) {
        new Thread() {
            @Override
            public void run() {
                try {
                    StatsEquipe nomEquipe = StatsEquipeDao.getNomEquipe(idEquipe);
                    modele.setNomEquipe(nomEquipe);

                    ((ProfileActivity)activite).runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if (nomEquipe!=null) {
                                ((ProfileActivity)activite).updateNomEquipe(nomEquipe);
                            }
                        }
                    });
                } catch (IOException e) {
